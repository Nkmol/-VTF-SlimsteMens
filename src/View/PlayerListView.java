...
public class PlayerListView extends JPanel implements Observer{
	...
	private void updatePlayers(ArrayList<ChallengedPlayer> players) {
		...
		for(int i = 0; i < players.size(); i++) {
			playerViews[i] = new PlayerView(players.get(i).getName(), players.get(i).isChallenged(), controller); // TODO add a check to see if the player has already been challenged.
			add(playerViews[i]);
		}
	}
